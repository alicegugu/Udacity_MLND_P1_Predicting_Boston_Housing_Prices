...
def explore_city_data(city_data):
    ...
    houses = np.array(housing_features)
    ...
    # Minimum price?
    print np.amin(prices, axis=0)
    # Maximum price?
    print np.amax(prices, axis=0)
    # Calculate mean price?
    print np.mean(prices, axis=0)
    # Calculate median price?
    print np.median(prices, axis=0)
...
def split_data(city_data):
    ...
    X_test = all[target+1:, :13]
    y_test = all[target+1:, 13:]
